...
The affine (rotation + scale + translation, 4 DOF) correctly models panning/rotation,
unlike pure translation which only handles straight-line camera movement.
"""
import logging
...
_MAX_FEATURES  = 2000
_RATIO_THRESH  = 0.75
_RANSAC_THRESH = 3.0   # pixels
_IDENTITY      = np.eye(2, 3, dtype=np.float32)   # fallback when match fails
...
def _match_affine(
    ...
    """
    Estimate affine M (2×3, float32) that maps img_a → img_b via SIFT + RANSAC.
    Returns (M, tx, ty).  Falls back to identity + (0, 0) on failure.
    """
    ...
    if des_a is None or des_b is None or len(kp_a) < 8 or len(kp_b) < 8:
        log.debug("VO: not enough keypoints (%d / %d)", len(kp_a or []), len(kp_b or []))
    ...
    if len(good) < 8:
        log.debug("VO: too few good matches (%d)", len(good))
        return _IDENTITY.copy(), 0.0, 0.0
    ...
    if M is None or inliers is None or int(inliers.sum()) < 8:
        log.debug("VO: RANSAC failed or too few inliers")
        return _IDENTITY.copy(), 0.0, 0.0
...
def compute_vo_offsets(
    ...
    Returns:
      x_offsets   : cumulative horizontal camera shift per frame (px, positive = right)
      y_offsets   : cumulative vertical shift (px)
      affine_mats : affine_mats[i] is the 2×3 matrix that maps frame i-1 → frame i
                    (affine_mats[0] = identity, unused)
    """
    n = len(frame_paths)
    x_offsets   = [0.0] * n
    y_offsets   = [0.0] * n
    ...
    for i in range(1, n):
        ...
        # Flip sign: positive tx means features moved right → camera moved LEFT
        # We want positive offset = camera panned right
