...
Stereo mode: StereoSGBM disparity → Z = (B × f) / disparity  [Equation 2].
Approximate mode: returns calib.approx_depth_m for every query (no stereo needed).
"""
import logging
...
# StereoSGBM defaults (tuned for ~848-wide images)
_SGBM_DEFAULTS = dict(
    minDisparity=0,
    numDisparities=64,
    blockSize=7,
...
class DepthEstimator:
    """
    Wraps either stereo disparity or a fixed approximate depth.
    Call get_depth(left_rect, right_rect, mask) → median Z in metres.
    """
    ...
    def get_depth(
        ...
        """
        Return median scene depth (m) in the masked region (or whole frame).
        In approximate mode, returns approx_depth_m regardless of inputs.
        """
        ...
        # Equation [2]: Z = (B × f) / disparity
        valid = disp > 1.0
        if mask is not None:
            valid &= mask.astype(bool)

        if not np.any(valid):
            log.debug("No valid disparity in mask; falling back to approx depth")
            return self._approx

        depth_map = (self._baseline * self._fx) / np.where(valid, disp, np.nan)
        median_z = float(np.nanmedian(depth_map[valid]))
        log.debug("Stereo median Z=%.2fm", median_z)
        return median_z
